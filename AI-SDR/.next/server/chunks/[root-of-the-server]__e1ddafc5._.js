...
(()=>{
    const e = new Error("Cannot find module '@/lib/lyzr'");
...
async function POST(request, { params }) {
    try {
        ...
        const strategyAgent = new LyzrAgent(process.env.LYZR_API_KEY || "", process.env.OUTREACH_STRATEGY_AGENT_ID || "");
        const strategyInput = {
            lead: {
                name: lead.name,
                title: lead.title,
            ...
            detected_signals: await __turbopack_context__.A("[project]/src/models/DetectedSignal.ts [app-route] (ecmascript, async loader)").then((mod)=>mod.default.find({
                    lead_id: leadId
                }))
        };
        const strategyResponse = await strategyAgent.chat(JSON.stringify(strategyInput));
        const strategyOutput = JSON.parse(strategyResponse);
        // Save strategy run
        ...
        const copyAgent = new LyzrAgent(process.env.LYZR_API_KEY || "", process.env.OUTREACH_COPY_AGENT_ID || "");
        const copyInput = {
            lead: {
                name: lead.name,
                title: lead.title,
        ...
        const copyResponse = await copyAgent.chat(JSON.stringify(copyInput));
        const copyOutput = JSON.parse(copyResponse);
        // Save copy run
...
//# sourceMappingURL=%5Broot-of-the-server%5D__e1ddafc5._.js.map